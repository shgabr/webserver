...
HTTPJPGService::HTTPJPGService(FileCache * p_fileCache,bool p_clean_cache )
        :HTTPService(p_fileCache,p_clean_cache) {}//constructor
bool HTTPJPGService::execute(HTTPRequest * p_httpRequest,TCPSocket * p_tcpSocket)
{//sets the headers of the response header to their appropriate values
    try {
        ...
        httpResponseHeader->setHeader("Content-Type","image/jpeg");//specifies the type of the resource
...
HTTPService * HTTPJPGService::clone ()
{//returns an object of the same type
    return new HTTPJPGService(fileCache,false);
}
HTTPJPGService::~HTTPJPGService()//destructor
{
}
