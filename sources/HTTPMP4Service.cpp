...
HTTPMP4Service::HTTPMP4Service(FileCache * p_fileCache,bool p_clean_cache )
        :HTTPService(p_fileCache,p_clean_cache) {}//constructor
bool HTTPMP4Service::execute(HTTPRequest * p_httpRequest,TCPSocket * p_tcpSocket)
{//sets the headers of the response header to their appropriate values
    try {
        ...
        httpResponseHeader->setHeader("Content-Type","video/mp4");//specifies the type of the resource
...
HTTPService * HTTPMP4Service::clone ()
{//returns an object of the same type
    return new HTTPMP4Service(fileCache,false);
}
HTTPMP4Service::~HTTPMP4Service()//destructor
{
}
