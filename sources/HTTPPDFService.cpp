...
HTTPPDFService::HTTPPDFService(FileCache * p_fileCache,bool p_clean_cache )
        :HTTPService(p_fileCache,p_clean_cache) {}//constructor
bool HTTPPDFService::execute(HTTPRequest * p_httpRequest,TCPSocket * p_tcpSocket)
{//sets the headers of the response header to their appropriate values
    try {
        ...
        httpResponseHeader->setHeader("Content-Type","application/pdf");//specifies the type of the resource
...
HTTPService * HTTPPDFService::clone ()
{//returns an object of the same type
    return new HTTPPDFService(fileCache,false);
}
HTTPPDFService::~HTTPPDFService()//destructor
{
}
