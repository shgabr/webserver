...
HTTPMP3Service::HTTPMP3Service(FileCache * p_fileCache,bool p_clean_cache )
        :HTTPService(p_fileCache,p_clean_cache) {}//constructor
bool HTTPMP3Service::execute(HTTPRequest * p_httpRequest,TCPSocket * p_tcpSocket)
{//sets the headers of the response header to their appropriate values
    try {
        ...
        httpResponseHeader->setHeader("Content-Type","audio/mp3");//specifies the type of the resource
...
HTTPService * HTTPMP3Service::clone ()//returns an object of the same type
{
    return new HTTPMP3Service(fileCache,false);
}
HTTPMP3Service::~HTTPMP3Service()//destructor
{
}
