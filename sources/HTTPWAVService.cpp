...
HTTPWAVService::HTTPWAVService(FileCache * p_fileCache,bool p_clean_cache )
        :HTTPService(p_fileCache,p_clean_cache) {}//constructor
bool HTTPWAVService::execute(HTTPRequest * p_httpRequest,TCPSocket * p_tcpSocket)
{//sets the headers of the response header to their appropriate values
    try {
        ...
        httpResponseHeader->setHeader("Content-Type","audio/wav");//specifies the type of the resource
...
HTTPService * HTTPWAVService::clone ()
{//returns an object of the same type
    return new HTTPWAVService(fileCache,false);
}
HTTPWAVService::~HTTPWAVService()//destructor
{
}
