...
HTTPTXTService::HTTPTXTService(FileCache * p_fileCache,bool p_clean_cache )
        :HTTPService(p_fileCache,p_clean_cache) {}//constructor
bool HTTPTXTService::execute(HTTPRequest * p_httpRequest,TCPSocket * p_tcpSocket)
{//sets the headers of the response header to their appropriate values
    try {
        ...
        httpResponseHeader->setHeader("Content-Type","text/plain");//specifies the type of the resource
...
HTTPService * HTTPTXTService::clone ()
{//returns an object of the same type
    return new HTTPTXTService(fileCache,false);
}
HTTPTXTService::~HTTPTXTService()//destructor
{
}
