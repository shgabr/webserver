...
HTTPPNGService::HTTPPNGService(FileCache * p_fileCache,bool p_clean_cache )
        :HTTPService(p_fileCache,p_clean_cache) {}//constructor
bool HTTPPNGService::execute(HTTPRequest * p_httpRequest,TCPSocket * p_tcpSocket)
{//sets the headers of the response header to their appropriate values
    try {
        ...
        httpResponseHeader->setHeader("Content-Type","image/png");//specifies the type of the resource
...
HTTPService * HTTPPNGService::clone ()
{//returns an object of the same type
    return new HTTPPNGService(fileCache,false);
}
HTTPPNGService::~HTTPPNGService()//destructor
{
}
